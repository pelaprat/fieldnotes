...
my $required = "<span style='font-size: 12px; color:red; font-style: italic;'>required</a>";

######################
## Log the activity ##
$lchc->log_user_activity( $user, { url    => 'admin/add.pl',
				   action => 'add',
			           target => $what } );
...
if($what eq 'course') {
    ...
    print $cgi->Tr($cgi->td({-class=>'label'}, 'Birthdate:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({ -name=>'birthdate', -size=>30, -value=>'' }), $cgi->br, 'yyyy-mm-dd'   ));
    print $cgi->Tr($cgi->td(), $cgi->td($cgi->submit({-name=>'submit', -value=>'submit'})));
    print $cgi->end_table();
    print $cgi->end_div();
    ...
    print $cgi->start_table();
    print $cgi->Tr($cgi->td($cgi->div({-style=>'width: 220px'})), $cgi->td());
    print $cgi->Tr($cgi->td({-class=>'label'}, 'First Name:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({-name=>'first', -size=>30, -value=>''}), $required ));
    print $cgi->Tr($cgi->td({-class=>'label'}, 'Middle Name:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({-name=>'middle', -size=>30, -value=>''})));
    print $cgi->Tr($cgi->td({-class=>'label'}, 'Last Name:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({-name=>'last', -size=>30, -value=>''}), $required ));
    ...
    print $cgi->Tr($cgi->td({-class=>'label'}, 'Email Address:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({-name=>'email', -size=>30, -value=>''}), $required ));
    print $cgi->Tr($cgi->td({-class=>'label'}, 'Password:'),
                   $cgi->td({-class=>'value'}, $cgi->textfield({-name=>'pass', -size=>30, -value=>''}), $required ));
