...
######################
## Log the activity ##
$lchc->log_user_activity( $user, { url    => 'admin/course.pl',
				   action => 'view',
			           target => 'course' } );
...
if($submit eq '' && $course <= 0) {
    ...
    print $cgi->b({ style => 'margin-left: 20px' }, 'Select the course (Only listing current courses):');
    print $cgi->p();

    ############################
    ## Get the right courses. ##
    my %sqlOptions = ( current => 1 );
    my %optionsOps = ( );
    my $query      = $db->sql_course( \%sqlOptions, \%optionsOps );
    my @results    = $db->complex_results( $query );

    ########################
    ## Print the results. ##
    foreach my $course ( @results ) {
	my $uri  = "$lchc->{admin_course}?course=$course->{id}&submit=select";
	my $text = "$course->{program} $course->{number} - $course->{name} [$course->{quarter} $course->{year}]";
	print $cgi->div({ -style => ('font-size: 14px; font-family: Georgia, Time New Roman, sans-serif;    '  .
				     ...
				     'margin-left: 20px; border: 2px solid #998888; cursor: pointer;        '  .
