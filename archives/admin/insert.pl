...
if($what eq 'site' && $name ne '') {
    $name = $lchc->safe_single_quotes($name);
    $db->simple_add($db->{tnSite}, ('null', "'$name'"));
}

## Add an activity?
if($what eq 'activity' && $name ne '') {
    $name = $lchc->safe_single_quotes($name);
    $db->simple_add($db->{tnActivity}, ('null', "'$name'"));
}
...
if($what eq 'space' && $name ne '' && $course > 0) {
    ...
    $db->simple_add($vftp->{tvCourseSpace}, ($course, $space)) if($space > 0);
}
