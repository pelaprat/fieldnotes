...
if($courseData->{current} != 1 && $type eq 'fieldnote') {
	...
	%menuOptions = ( selected => 39, selectedOp => '==' );
	...
	print $lchc->site_menu('site', \%menuOptions, \%sqlOptions, {});
        print $cgi->p;
	...
	$lchc->js_site_activity_menu( 'activity', {}, 39 );
	...
	print $cgi->textarea({-name=>'narrative', -rows=>11, -columns=>40});
        print $cgi->p;

        print $cgi->div({-class=>'box-title'}, 'III. Game-task Level Summary / Letter to Wizard');
        print $cgi->textarea({-name=>'gametask', -rows=>11, -columns=>40});
        print $cgi->p;

        print $cgi->div({-class=>'box-title'}, 'IV. Reflection');
        print $cgi->textarea({-name=>'reflection', -rows=>12, -columns=>40});
	print $cgi->end_div();

	## -onClick=>'return checkForm(document.forms[0]);',
	print $cgi->submit({-name=>'submit', -value=>"submit $type",
			    -style=>'text-transform: uppercase; border: 2px solid #000000; width: 200px'});
