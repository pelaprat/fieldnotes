...
$year_begin  = $lchc->{_year_base}    if ! defined $year_begin;
$day_end     = 1                      if ! defined $day_end;
$month_end   = 1                      if ! defined $month_end;
$year_end    = $lchc->{_year_current} if ! defined $year_end || ( $year_end == $year_begin );
...
######################
## Log the activity ##
$lchc->log_user_activity( $user, { url    => 'search/results', 
				   action => 'view' });
...
#     $year_begin  == $lchc->{_year_base} && $year_end  == $lchc->{_year_base} )) {
